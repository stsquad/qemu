...
/*
 * This helper is a trampoline call as we need to resolve haddr before
 * calling the eventual plugin helpers. This avoids us having to save
 * the haddr somewhere in generated code. The overhead should be
 * mitigated by a hot cache.
 */
...
{
    void *haddr = NULL;
    qemu_plugin_vcpu_mem_haddr_cb_t cb = (qemu_plugin_vcpu_mem_haddr_cb_t) callback;
    cb(vcpu_index, info, vaddr, haddr, userdata);
}
