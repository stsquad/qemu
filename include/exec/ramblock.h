...
struct RAMBlock {
    ...
    uint8_t *colo_cache;
    /** @offset: offset into host ram_addr_t address space */
    ram_addr_t offset;
    ...
    /**
     * @fd: fd of backing store if used, for anonymous RAMBlocks
     * it's always -1
     */
    int fd;
    ...
    uint64_t fd_offset;
    /**
     * @page_size: backend page size of backing store, will be
     * PAGE_SIZE for anonymous RAM otherwise depends on file-system
     */
    size_t page_size;
    ...
    /*
     * The following fields are only used by mapped-ram migration
     */

    /** @file_bmap: bitmap of pages present in the migration file  */
    unsigned long *file_bmap;
    /** @bitmap_offset: offset in the migration file of the bitmap */
    off_t bitmap_offset;
