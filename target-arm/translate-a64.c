...
static void handle_ldst_reg_unsigned_imm(DisasContext *s, uint32_t insn)
{
    ...
    bool is_store, is_signed, is_extended;
...
static void disas_movw_imm(DisasContext *s, uint32_t insn)
{
    ...
    /* ensure we fault if a 32 bit op tries to shift too much */
    if (!sf && (pos==32 || pos==48)) {
        unallocated_encoding(s);
        return;
    }
 ...
 *        (for 32bit shifting more than 32 will cause an undef)
 */
                       ...
                       unsigned int rm, unsigned int rn, unsigned int rd)
{
    ...
    if (!sf && is_signed) {
        tcg_n = new_tmp_a64_zero(s);
        tcg_m = new_tmp_a64_zero(s);
