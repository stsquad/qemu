...
static GOptionEntry options[] =
{
    { "socket-path", 0, 0, G_OPTION_ARG_FILENAME, &socket_path,
      ...
      "PATH" },
    { "flash-path", 0, 0, G_OPTION_ARG_FILENAME, &flash_path,
      "Location of raw flash image file",
      "PATH" },
    ...
    { "fd", 0, 0, G_OPTION_ARG_INT, &socket_fd,
      "Specify the file-descriptor of the backend, incompatible with --socket-path",
      "FD" },
...
static int vrpmb_process_msg(VuDev *dev, VhostUserMsg *msg, int *do_reply)
{
    ...
    g_info("%s: msg %s(%d)", __func__,
           vu_request_to_string(msg->request), msg->request);
...
static const VuDevIface vuiface = {
    ...
    .get_protocol_features = vrpmb_get_protocol_features,
